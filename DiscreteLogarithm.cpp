...
class ZZ {
 public:
  ...
  ZZ(string const& str, int base = 10) {
    mpz_init_set_str(data, str.c_str(), base);
  }

  ~ZZ() noexcept { mpz_clear(data); }

  ZZ& operator=(ZZ const& a) {
    mpz_set(data, a.data);
    return *this;
  }  // self-assignment ok

  ZZ& operator+=(ZZ const& a) {
    mpz_add(data, data, a.data);
    return *this;
  }
  ZZ& operator-=(ZZ const& a) {
    mpz_sub(data, data, a.data);
    return *this;
  }
  ZZ& operator*=(ZZ const& a) {
    mpz_mul(data, data, a.data);
    return *this;
  }
  ...
  ZZ operator-() const {
    ZZ res;
    mpz_neg(res.data, data);
    return res;
  }

  ZZ& operator/=(ZZ const& a) {
    mpz_fdiv_q(data, data, a.data);
    return *this;
  }
  ZZ& operator%=(ZZ const& a) {
    mpz_fdiv_r(data, data, a.data);
    return *this;
  }
  ...
  ZZ operator^(unsigned long int e) const {
    ZZ res;
    mpz_pow_ui(res.data, data, e);
    return res;
  }
  ...
  friend ostream& operator<<(ostream& out, ZZ const& a) {
    out << a.ToString();
    return out;
  }

  // friends
  friend ZZ Gcd(ZZ const& a, ZZ const& b) {
    ZZ res;
    mpz_gcd(res.data, a.data, b.data);
    return res;
  }
  friend ZZ PowMod(ZZ const& a, ZZ const& e, ZZ const& n) {
    ZZ res;
    mpz_powm(res.data, a.data, e.data, n.data);
    return res;
  }
  friend ZZ InvMod(ZZ const& a, ZZ const& n) {
    ZZ res;
    if (!mpz_invert(res.data, a.data, n.data)) return 0;
    return res;
  }
  friend bool IsPrime(ZZ const& n, int reps = 10) {
    return (mpz_probab_prime_p(n.data, reps) > 0);
  }
  friend ZZ Sqrt(ZZ const& a) {
    ZZ res;
    mpz_sqrt(res.data, a.data);
    return res;
  }

  // random
  static void Seed(unsigned long int seed) { gmp_randseed_ui(state.s, seed); }
  friend ZZ Random(ZZ const& n) {
    ZZ res;
    mpz_urandomm(res.data, ZZ::state.s, n.data);
    return res;
  }

  // other
  bool operator[](size_t index) const {
    return mpz_tstbit(data, (mp_bitcnt_t)index);
  }
  ...
  class RandState {
   public:
    RandState() {
      ...
      gmp_randseed_ui(s, 0xf0123456789abcdeul);
    }
...
ZZ babyGiant(ZZ b, ZZ a, ZZ n, ZZ p) {
  ...
  while (i <= m) {
    if (hsTb.find(g) != hsTb.end()) return i * m + hsTb[g];
    g *= am;
    g %= p;
    i += 1;
  }
...
ZZ discLog(ZZ b, ZZ a, ZZ ord, ZZ p) {
  ZZ m = pollard(ord);
  if (ord == m) return babyGiant(b, a, ord, p);
  ZZ n = ord / m;
  ZZ x2 = discLog(PowMod(b, n, p), PowMod(a, n, p), m, p);
